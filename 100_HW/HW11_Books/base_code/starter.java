...
class starter {
		...
		*/
		while(true){
			...
			if (choice.equals("file")){
				...
				System.out.println("There is/are "+checksArray(arr,word)+" \""+word+"\" in this file");
			}
			else if (choice.equals("common")){
				System.out.println("The most common word is: "+mostCommon(arr));
			}
			else {
				System.out.println("Try again..Invalid response");
			}
	...
	*/
	public static int checksArray(String[] arr, String check){
		...
		return occurence;	// Dummy value, delete when needed.
	}
	...
	*/
	public static String mostCommon(String[] arr){
		...
		for (int i=0; i<arr.length-1;i++){
			int add = 1;
			int dups = 0;
			while (i+add<arr.length){
				if(arr[i].equals(arr[i+add])){
					dups++;
				}
				add++;
		...
		return maxWord;	// Dummy value, delete when needed.
	}
